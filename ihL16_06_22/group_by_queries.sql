...
select   distinct last_name -- , actor_id
 from actor
 group by actor_id
order by last_name;

select   actor_id, last_name -- , actor_id
 from actor
 group by actor_id
order by last_name;
...
SELECT last_name, last_name, COUNT(last_name) as counts
FROM actor
GROUP BY last_name
HAVING  counts > 1;

SELECT s.first_name, s.last_name, COUNT(r.rental_id) as num_rentals, staff_id
FROM staff  as s
...
SELECT COUNT( distinct title) as num_titles , rating
FROM film
GROUP BY rating
ORDER BY rating ;
